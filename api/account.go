...
func (server *Server) createAccount(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&req); err != nil {
		ctx.JSON(http.StatusBadRequest, errorResponase(err))
		return
	}
...
func (server *Server) getAccount(ctx *gin.Context) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			ctx.JSON(http.StatusNotFound, errorResponase(err))
		}
		ctx.JSON(http.StatusInternalServerError, errorResponase(err))
		return
	}
